...
describe('Task', () => {
  ...
  const buildErc20Calls = (balanceWBTC: string, balanceWETH: string, balanceDAI: string): ContractCall[] => [
    // WBTC
    { to: WBTC, chainId: 10, data: '0x70a08231', output: balanceWBTC, outputType: 'uint256' }, // balanceOf(user)
    { to: WBTC, chainId: 10, data: '0x313ce567', output: '8', outputType: 'uint8' }, // decimals
    // WETH
    { to: WETH, chainId: 10, data: '0x70a08231', output: balanceWETH, outputType: 'uint256' },
    { to: WETH, chainId: 10, data: '0x313ce567', output: '18', outputType: 'uint8' },
    // DAI
    { to: DAI, chainId: 10, data: '0x70a08231', output: balanceDAI, outputType: 'uint256' },
    { to: DAI, chainId: 10, data: '0x313ce567', output: '18', outputType: 'uint8' },
  ]

  describe('when there are some balances', () => {
    // Prices: BTC=$60k, ETH=$3k, DAI=$1 — all with 1e18 USD precision
    const prices = [
      { token: WBTC, chainId: 10, usdPrice: '60000000000000000000000' }, // 60000 * 1e18
      { token: WETH, chainId: 10, usdPrice: '3000000000000000000000' }, // 3000  * 1e18
      { token: DAI, chainId: 10, usdPrice: '1000000000000000000' }, // 1     * 1e18
    ]
